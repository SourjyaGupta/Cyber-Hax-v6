...
async function startServer() {
  ...
  // Serve the landing site at the root
  app.use(express.static(path.join(root, "landing_site")));

  // Serve the game at /play
  app.use("/play", express.static(path.join(root, "web")));
  ...
  // Fallback for landing site
  app.get("*", (req, res) => {
    res.sendFile(path.join(root, "landing_site", "index.html"));
  });
