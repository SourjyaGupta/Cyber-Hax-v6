...
function defaultServerBase() {
  // Public itch build always targets the hosted Render backend.
...
async function createRoomAndConnect() {
  ...
  try {
    const selectedServerBase = defaultServerBase();
    ...
    // If room creation is temporarily unavailable, players can still coordinate via a manual room code.
    updateJoinStatus("Create Room is unavailable right now. Paste a room code from your opponent and press Join Room.", "warning");
...
function bootstrap() {
  ...
  state.serverBase = defaultServerBase();
  els.playerInput.value = state.playerName;
  els.sessionInput.value = state.sessionName;
  els.serverInput.value = state.serverBase;
